...
local FFLAGS = {
	...
	-- fixes wallclipping, by picking the best wall depending on Mario's
	-- face angle. (See: https://youtu.be/TSCzC-WECw8?t=42)
	FIX_WALLCLIPPING = false,
	...
	-- Add a small unit so walls always get found when Mario is not moving near a wall.
	WALL_MARGIN = 0.1,
	------------------------------------------------------
	...
	-- the rate the game ticks at per second
	STEP_RATE = 30,
	
	-- if the game should emulate certain limits like level boundaries
	...
	-----------------------VISUAL-----------------------
	-- if animations should smoothen themselves when changing animation
	ANIMATION_SMOOTHENING = false,
	
	-- If the rendered character should have the position interpolated between frames.
	CHARACTER_INTERPOLATION = true,
	----------------------------------------------------

	-----------------------DEBUG-----------------------
	-- if debug toggling is enabled
	DEBUG_TOGGLE = true,
	
	-- to debug or not to debug
	DEBUG = false,
	
	-- If faces Mario (can) collide with are visualized. yrah
	DBG_COLL_FACE = false,
	
	-- if rays shold be debuged (causes lag when showing too much rays)
	RAY_DBG = true,
