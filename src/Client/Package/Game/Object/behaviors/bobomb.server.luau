...
-- local MODEL.EXPLOSION, MODEL.BLACK_BOBOMB, MODEL.SMOKE = MODEL.EXPLOSION, MODEL.BLACK_BOBOMB, MODEL.SMOKE
...
--
local INT_SUBTYPE = Enums.Interaction.Subtype
local INT_SUBTYPE2 = Enums.Interaction.Method
local INT_STATUS = Enums.Interaction.Status
INT_SUBTYPE_NPC, INT_SUBTYPE_KICKABLE, INTERACT_GRABBABLE = INT_SUBTYPE.NPC, INT_SUBTYPE.KICKABLE, INT_SUBTYPE2.GRABBABLE
--
...
local function bhv_bobomb_init(o)
	...
	o.InteractionSubtype:Set(INT_SUBTYPE_KICKABLE)
end
...
local function bobomb_act_patrol(o)
	...
	local collisionFlags = object_step(o)
	if (obj_return_home_if_safe(o, o.Home.X, o.Home.Y, o.Home.Z, 400) == true)
		and (obj_check_if_facing_toward_angle(o.MoveAngleYaw, o.AngleToMario, 0x2000) == true) then
		print('wow')
		print('AAHH!')
		o.BobombFuseLit = 1
...
local function bobomb_act_explode(o)
	...
	if o.Timer < 5 then
		...
		local explosion = spawn_object(o, MODEL.EXPLOSION, 'bhvExplosion')
		explosion.GraphYOffset += 100.0

		bobomb_spawn_coin(o)
		create_respawner(MODEL.BLACK_BOBOMB, 'bhvBobomb', 3000)
		o.ActiveFlags:Set(ACTIVE_FLAGS_DEACTIVATED) --/ unload object
	end

end

local function bobomb_act_launched(o)
	--local o = ObjectListProcessor.gCurrentObject
...
local function bobomb_check_interactions(o)
	...
	if o.InteractStatus:Has(INT_STATUS_INTERACTED) then
		...
		--if (o.InteractStatus & INT_STATUS_TOUCHED_BOB_OMB) ~= false) then
...
local function bhv_bobomb_loop(o)
	...
	if is_point_within_radius_of_mario(o.Position.X, o.Position.Y, o.Position.Z, 4000) ~= false then
		...
		curr_obj_random_blink(o, 'BobombBlinkTimer')
		...
		if o.BobombFuseLit == 1 then
			...
			--if (dustPeriodMinus1 & o.BobombFuseTimer) == 0 then
				spawn_object(o, MODEL.SMOKE, 'bhvBobombFuseSmoke')
			end

			cur_obj_play_sound_1(o, Sounds.AIR_BOBOMB_LIT_FUSE)
...
local function bhv_bobomb_buddy_init(o)
	...
	o.InteractionSubtype:Set(INT_SUBTYPE_NPC)
end
