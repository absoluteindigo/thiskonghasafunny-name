...
local BlinkStates = {
	[0] = 'rbxassetid://73937784514070',
	[1] = 'rbxassetid://125027146211150',
}
...
-- local scaleYOffset = -(0.597 * 1.5) -- -1.31
local scaleYOffset = -1.31
local defaultScale = 1.5
local shadowSize = 7.6 / defaultScale
...
function class:New(ModelClass)
	...
	local reference = ReferenceHelper.new(newModel, function(self, instance: Instance)
		if self.TorsoBillboard and self.Face then return end
		-- instance = newModel:FindFirstChild('TORSO', true)
		
		if instance.Name == 'FACE' and instance:IsA('MeshPart') then
			self.Face = instance
	...
	-- shadow
	local scale = Link.GoombaScale or 1
	ModelClass.lastScale = scale
	ModelClass.shadow = Shadow.new(shadowSize * scale)
...
function custom.Update(self)
	...
	if not isHidden then
		...
		if self.lastScale ~= scale then
			...
			self.shadow:changeSize(shadowSize * scale)
		end]]
		self.shadow:update(
		...
		if scale ~= self.Scale then
			self.Scale = scale
			self.shadow:changeSize(shadowSize * scale.X)
