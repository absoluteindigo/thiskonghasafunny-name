...
local Object = require(ClientCore.Game.Object)
local BehaviorScripts = Object.BehaviorData.BehaviorScripts
...
local function add_tree_leaf_particles(m: Mario)
	local leafHeight;

	if Util.CompareBhvScripts(m.UsedObj.behavior, BehaviorScripts.bhvTree) then
		-- if m.usedObj.behavior == gLinker.behaviors.bhvTree then
		warn('Area.gCurrLevelNum == LEVEL_SSL')
		-- CUSTOM CODE HERE
		local Model = m.UsedObj.Model
		if Model and Model.Name == 'SSL_PALM_TREE' then
			leafHeight = 250.0
		else
			leafHeight = 100.0
		end
		-- END CUSTOM CODE
		--[[if Area.gCurrLevelNum == LEVEL_SSL then
			leafHeight = 250.0
		else
			leafHeight = 100.0
		end]]

		if m.Position.Y - m.FloorHeight > leafHeight then
			m.ParticleFlags:Add(ParticleFlags.LEAF)
...
-- beautiful, no hack, stay
local function getPoleValues(m: Mario): (number, Vector3)
	return m.UsedObj.HitboxHeight, m.UsedObj.Position
...
local function setPolePosition(m: Mario, offsetY: number): number
	...
	elseif m.PolePos < -m.UsedObj.HitboxDownOffset then
		-- ok WHY WOULD YOU comment out hitboxDownOffset
		-- i hate this code so bad
...
local function playClimbingSounds(m: Mario, b: number)
	local isOnTree = Util.CompareBhvScripts(m.UsedObj.behavior, BehaviorScripts.bhvTree) -- m.UsedObj.behavior == BehaviorScripts.bhvTree -- false
	print(isOnTree)
	...
	if b == 1 then
		if m:IsAnimPastFrame(1) then
			-- play_sound(isOnTree and Sounds.ACTION_CLIMB_UP_TREE or Sounds.ACTION_CLIMB_UP_POLE, m.marioObj.gfx.cameraToObject)
			m:PlaySound(isOnTree and Sounds.ACTION_CLIMB_UP_TREE or Sounds.ACTION_CLIMB_UP_POLE)
		end
	else
		-- play_sound(isOnTree and Sounds.MOVING_SLIDE_DOWN_TREE or Sounds.MOVING_SLIDE_DOWN_POLE, m.marioObj.gfx.cameraToObject)
