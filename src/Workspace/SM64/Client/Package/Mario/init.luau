...
local Audio = require(Custom.Audio)
...
local GraphNodeConstats = require(SM64.Game.Object.GraphNodeConstats)
local SpawnSound = require(SM64.Game.Object.SpawnSound) -- temporary use case
...
local overwrittenActions: { MarioAction } = {}
Mario.OverwrittenActions = overwrittenActions
...
function Mario.RegisterAction(actionType: number, action: MarioAction)
	if actions[actionType] then
		...
		if not overwrittenActions[actionType] then
			overwrittenActions[actionType] = actions[actionType]
		end
	end
...
function Mario.RevertOverwrittenActions()
	for k: any, action: MarioAction in pairs(overwrittenActions) do
		actions[k] = action
	end
	table.clear(overwrittenActions)
end
...
function Mario.PlaySound(self: Mario, sound: Instance?)
	if not sound then
		return
	end
	local AudioClass = self.Audio
	
	-- assert(sound) no reason to lol

	if sound:IsA("Sound") then
		AudioClass:Play(sound)
...
function Mario.UpdatePunchSequence(m: Mario)
	...
	if actionArg == 0 or actionArg == 1 then
		...
		if m.ActionArg == 2 then
			m.BodyState.PunchState = bit32.bor(bit32.lshift(0, 6), 4)
		end
	...
	elseif actionArg == 3 or actionArg == 4 then
		...
		if m.ActionArg == 5 then
			m.BodyState.PunchState = bit32.bor(bit32.lshift(1, 6), 6)
		end
	...
	elseif actionArg == 6 then
		...
		if animFrame == 0 then
			m.BodyState.PunchState = bit32.bor(bit32.lshift(2, 6), 6)
		end
...
function Mario.PerformGroundQuarterStep(m: Mario, nextPos: Vector3): number
	...
	if FFlags.FIX_WALLCLIPPING then
...
type sm64vec3 = {[number]: number}
type sm64matrix = {[number]: sm64vec3}
local function get_pos_from_transform_mtx(dest: sm64vec3, objMtx: sm64matrix, camMtx: sm64matrix)
	local camX = camMtx[3][0] * camMtx[0][0] + camMtx[3][1] * camMtx[0][1] + camMtx[3][2] * camMtx[0][2]
	local camY = camMtx[3][0] * camMtx[1][0] + camMtx[3][1] * camMtx[1][1] + camMtx[3][2] * camMtx[1][2]
	local camZ = camMtx[3][0] * camMtx[2][0] + camMtx[3][1] * camMtx[2][1] + camMtx[3][2] * camMtx[2][2]
...
function Mario.SwitchHandGrabPos(m: Mario, callContext: string)
	...
	-- lol i know what callcontext is
	return nil
end
...
function Mario.UpdateCaps(m: Mario): Flags
	...
	-- https://github.com/sm64js/sm64js/blob/04c1a984117ebb8d0e7b0d5d2e3424367f69b92d/src/game/Mario.js#L2082
	-- this is innacurate (no surprise tbh)
function Mario.mario_update_hitbox_and_cap_model(m: Mario)
	...
	if m.InvincTimer >= 3 and bit32.band(Util.GlobalTimer, 1) > 0 then
		...
		m.GfxFlags:Add(GraphNodeConstats.GRAPH_RENDER_INVISIBLE)
	...
	-- MARIO_TELEPORTING
	-- idk why i made fadeWarpOpacity into FadeWarpOpacity
	if m.Flags:Has(MarioFlags.TELEPORTING) and m.FadeWarpOpacity ~= 0xFF then
		modelState.Value = bit32.band(modelState.Value, bit32.bnot(0xFF))
		-- modelState &= ~0xFF
		modelState.Value = bit32.bor(modelState.Value, bit32.bor(0x100, m.FadeWarpOpacity))

		-- modelState |= (0x100 | m.fadeWarpOpacity)
    	end
end

-- raghhh i dont even care dude
Mario.UpdateModel = Mario.mario_update_hitbox_and_cap_model
...
function Mario.CopyMarioStateToObject(m: Mario, o: Types.ObjectState)
	...
	-- https://github.com/sm64js/sm64js/blob/04c1a984117ebb8d0e7b0d5d2e3424367f69b92d/src/game/ObjectListProcessor.js#L159
	-- why
...
function Mario.ExecuteAction(m: Mario): number
	...
	while m.Action() > 0 do
		...
		if action then
			...
			elseif group == ActionGroups.SUBMERGED then
				...
				if not cancel then
					m.QuicksandDepth = 0
					m.BodyState.HeadAngle = Vector3int16.new(m.BodyState.HeadAngle.X, 0, 0)
				end
	...
	m:UpdateModel() -- mario_update_hitbox_and_cap_model?
...
function Mario.new(): Mario
	...
	local state: MarioState = {
		...
		-- Hacky solutions...
		-- PoleObj = nil,
		PoleYawVel = 0, -- these down here litterally arent even hacky 
		PolePos = 0,
		
		-- todo: make this an object IG
		IntangibleTimer = 0,
		InteractStatus = Flags.new(),
		InteractionSubtype = Flags.new(),
		ActiveFlags = Flags.new(Enums.Object.ActiveFlag.ACTIVE, Enums.Object.ActiveFlag.UNK8),
		Room = -1,
		GfxFlags = Flags.new(GraphNodeConstats.GRAPH_RENDER_ACTIVE),
		InteractType = 0,
		FaceAnglePitch = 0,
		FaceAngleYaw = 0,
		FaceAngleRoll = 0,
		CollidedObjInteractTypes = Flags.new(),
	}
	
	state.Audio = Audio.new(state)
...
function Mario.Init(m: Mario, maybeSpawnInfo: dict?): Mario
	...
	m.BodyState.PunchState = 0 -- yesss
